...
IWD.OPC = {
		...
		initMessages: function(){
			$j(document).on('click touch', '.close-message-wrapper, .opc-messages-action .button', function(){
		...
		initSaveOrder: function(){
			...
			$j(document).on('click touch', '.opc-btn-checkout', function(){
		...
		initPayment: function(){
			...
			$j(document).on('click touch', '#co-payment-form input[type="radio"]', function(event){
				IWD.OPC.removeNotAllowedPaymentMethods();
...
IWD.OPC.Checkout = {
		...
		reloadShippingsPaymentsNEED: false,
		reloadShippingsPaymentsCnt: 0,
		...
		prepareShippingMethodResponse: function(response){
			$j('#warningDropshipMessage').remove();
			IWD.OPC.Checkout.xhr = null;
			if (typeof(response.error)!="undefined"){
...
//				IWD.OPC.recheckAgree();
			...
			if (typeof response == 'string') {
				...
				if (typeof(response.error) != "undefined") {
					$j('#checkout-shipping-method-load').append(
						'<div id="warningDropshipMessage" style="color: #F90000;" class="warning-message">' + response.message + '</div>');
					IWD.OPC.saveOrderStatus = false;
				}
			}
		...
		hideLoader: function(){
			setTimeout(function(){
				clearTimeout(IWD.OPC.Checkout.loaderCntChecker);
				IWD.OPC.Checkout.loaderCnt--;
				if(IWD.OPC.Checkout.loaderCnt<1) {
					$j('.opc-ajax-loader').hide();
				}
				IWD.OPC.Checkout.loaderCntChecker = setTimeout(IWD.OPC.Checkout.checkLoaderCnt,1000);
				//$j('.opc-btn-checkout').removeClass('button-disabled');				
			},600);
		},

	    checkLoaderCnt: function () {
			if(IWD.OPC.Checkout.loaderCnt>0){
				IWD.OPC.Checkout.hideLoader();
			}
		},
...
//					IWD.OPC.recheckAgree();
		...
		reloadShippingsPayments: function(form_type, delay){
			...
			IWD.OPC.Checkout.reloadShippingsPaymentsCnt++;
			IWD.OPC.Checkout.showLoader();
			IWD.OPC.Checkout.lockPlaceOrder();
		...
		reloadShippingsPaymentsResponse: function(response){
			...
			if (typeof(response.shipping) != "undefined"){
				$j('#shipping-block-methods').empty().html(response.shipping);
				if($j('#shipping-block-methods #co-shipping-method-form').length==0){
					IWD.OPC.Checkout.reloadShippingsPaymentsCnt = 0;
					IWD.OPC.Checkout.hideLoader();
					IWD.OPC.Checkout.unlockPlaceOrder();
				}else if($j('#shipping-block-methods #co-shipping-method-form').length>0 && IWD.OPC.Checkout.reloadShippingsPaymentsNEED){
					if(IWD.OPC.Checkout.reloadShippingsPaymentsCnt<4) {
						if (IWD.OPC.Checkout.xhr2!=null) {
							IWD.OPC.Checkout.xhr2.abort();
						}
						IWD.OPC.Checkout.xhr2 = IWD.OPC.Checkout.reloadShippingsPayments('shipping', 2000);
					}else {
						$j('#co-shipping-method-form').html('<srtong>Sorry, no quotes are available for this order at this time.</srtong>');
						IWD.OPC.Checkout.hideLoader();
						IWD.OPC.Checkout.unlockPlaceOrder();
					}
				}else{
...
IWD.OPC.Billing = {
		...
		init: function(){
			...
			$j(document).on('click touch', 'input[name="billing[create_account]"]', function(){
				if ($j(this).is(':checked')){
...
IWD.OPC.Coupon = {
		init: function(){
			...
			$j(document).on('click touch', '.apply-coupon', function(){
				IWD.OPC.Coupon.applyCoupon(false);
			});
			
			
			$j(document).on('click touch', '.remove-coupon', function(){
				IWD.OPC.Coupon.applyCoupon(true);
			});
			
			
			$j(document).on('click touch','.discount-block h3', function(){
...
IWD.OPC.Comment = {
		init: function(){
			...
			$j(document).on('click touch','.comment-block h3', function(){
				if ($j(this).hasClass('open-block')){
					$j(this).removeClass('open-block');
					$j(this).next().addClass('hidden');
				}else{
...
IWD.OPC.Gift = {
		init: function(){
			...
			$j(document).on('click touch','.gift-messages label', function(){
				if ($j(this).hasClass('open-block')){
					$j(this).removeClass('open-block');
					$j(this).next().addClass('hidden');
				}else{
...
IWD.OPC.SignatureAtCheckout = {
    init: function(){
        $j(document).on('click touch','.signature-block h3', function(){
            if ($j(this).hasClass('open-block')){
                $j(this).removeClass('open-block');
                $j(this).next().addClass('hidden');
            }else{
...
IWD.OPC.Agreement ={
		...
		init: function(){
			...
			$j(document).on('click touch', '.view-agreement', function(e){
				e.preventDefault();
			...
			$j(document).on('click touch', '#checkout-agreements input[name*="agreement"]', function(){
				var cur_el = $j(this);
				$j('#checkout-agreements input').each(function(){
...
IWD.OPC.Login ={
		...
		init: function(){
			$j(document).on('click touch', '.login-trigger', function(e){
				e.preventDefault();
				$j('#modal-login').addClass('md-show');
			});

			$j(document).on('click touch','.md-modal .close', function(e){
				e.preventDefault();
				$j('.md-modal').removeClass('md-show');
			});

			$j(document).on('click touch', '.restore-account', function(e){
				e.preventDefault();
				$j('#login-form').hide();$j('#login-button-set').hide();
				$j('#form-validate-email').fadeIn();$j('#forgotpassword-button-set').show();
			});

			$j(document).on('click touch', '#login-button-set .btn', function(){
				$j('#login-form').submit();
			});
			$j(document).on('click touch', '#forgotpassword-button-set .btn', function(){
			...
			$j(document).on('click touch', '#forgotpassword-button-set .back-link', function(e){
				e.preventDefault();
...
IWD.OPC.Decorator = {
		initReviewBlock: function(){
			$j(document).on('click touch','a.review-total',function(e){
		...
		setActivePayment: function(){
			...
			$j(document).on('click touch','#checkout-payment-method-load dt', function(){
				$j('#checkout-payment-method-load dt').removeClass('active');
				$j(this).addClass('active');
			});
		},

		initSpecialCheckboxes: function () {
			/*---------- Subscribe ---------------*/
			if ($j('#is_subscribed').is(':checked'))
				$j('#is_subscribed').parent().prepend('<label for="is_subscribed" class="label-for-design-checkbox"><i class="fa fa-check-square"></i></label>');
			else
				$j('#is_subscribed').parent().prepend('<label for="is_subscribed" class="label-for-design-checkbox"><i class="fa fa-square-o" class="label-for-design-checkbox"></i></label>');

			if ($j('.form-list .create_account').is(':checked'))
				$j('.form-list .create_account').parent().prepend('<label for="billing:create_account" class="label-for-design-checkbox"><i class="fa fa-check-square"></i></label>');
			else
				$j('.form-list .create_account').parent().prepend('<label for="billing:create_account" class="label-for-design-checkbox"><i class="fa fa-square-o"></i></label>');

			if ($j('.form-list .use_for_shipping_yes').is(':checked'))
				$j('.form-list .use_for_shipping_yes').parent().prepend('<label for="billing:use_for_shipping_yes" class="label-for-design-checkbox"><i class="fa fa-check-square"></i></label>');
			else
				$j('.form-list .use_for_shipping_yes').parent().prepend('<label for="billing:use_for_shipping_yes" class="label-for-design-checkbox"><i class="fa fa-square-o"></i></label>');


			if ($j('.form-list .save_in_address_book').is(':checked'))
				$j('.form-list .save_in_address_book').parent().prepend('<label for="billing:save_in_address_book" class="label-for-design-checkbox"><i class="fa fa-check-square"></i></label>');
			else
				$j('.form-list .save_in_address_book').parent().prepend('<label for="billing:save_in_address_book" class="label-for-design-checkbox"><i class="fa fa-square-o"></i></label>');

			if ($j('.form-list .save_in_address_book_shipping').is(':checked'))
				$j('.form-list .save_in_address_book_shipping').parent().prepend('<label for="shipping:save_in_address_book" class="label-for-design-checkbox"><i class="fa fa-check-square"></i></label>');
			else
				$j('.form-list .save_in_address_book_shipping').parent().prepend('<label for="shipping:save_in_address_book" class="label-for-design-checkbox"><i class="fa fa-square-o"></i></label>');
		},

		bindEventSpecialCheckboxes: function () {
			$j(document).on('click touch','.opc-newsletter .control .checkbox',function(){
				if ($j(this).is(':checked')) {
					$j(this).parent().find('i').attr('class', '');
					$j(this).parent().find('i').addClass('fa fa-check-square');
				}
				else {
					$j(this).parent().find('i').attr('class', '');
					$j(this).parent().find('i').addClass('fa fa-square-o');
				}
			});

			$j(document).on('click touch','.form-list .create_account',function(){
				if ($j(this).is(':checked')) {
					$j(this).parent().find('i').attr('class', '');
					$j(this).parent().find('i').addClass('fa fa-check-square');
				}
			...
			$j(document).on('click touch','.form-list .use_for_shipping_yes',function(){
				if ($j(this).is(':checked')) {
					$j(this).parent().find('i').attr('class', '');
					$j(this).parent().find('i').addClass('fa fa-check-square');
				}
			...
			$j(document).on('click touch','.form-list .save_in_address_book',function(){
				if ($j(this).is(':checked')) {
					$j(this).parent().find('i').attr('class', '');
					$j(this).parent().find('i').addClass('fa fa-check-square');
				}
			...
			$j(document).on('click touch','.form-list .save_in_address_book_shipping',function(){
				if ($j(this).is(':checked')) {
					$j(this).parent().find('i').attr('class', '');
					$j(this).parent().find('i').addClass('fa fa-check-square');
				}
...
$j(document).ready(function(){
	...
	IWD.OPC.Decorator.initSpecialCheckboxes();
	IWD.OPC.Decorator.bindEventSpecialCheckboxes();
});
