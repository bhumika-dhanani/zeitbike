                    ...
                    if(data.length>0) //results found
                    {
                        for(var i = 0; i < data.length; i++)
                        {
                            ...
                            $($autocomplete).getElementsBySelector('ul').each(function(elem) {
                                ...
                                var afmetingHtml = '';
                                if (start2 >= 0) {
                                    $keyword2 = data[i].sku.slice(start2, end2);
                                }
                                if (data[i].afmeting) {
                                    afmetingHtml += '<span class="product-afmeting">Afmeting: '+data[i].afmeting+'</span><br>';
                                }
                                $(elem).update($(elem).innerHTML+'<li class="'+$class+'">'+
                                    ...
                                    '<input class="increment no-display" value="'+data[i].increment +'" />'+
                                    '<input class="minsale no-display" value="'+data[i].minsale +'" />'+
 ...
 */
 ...
 {  
    ...
    var $afmeting = '';
    $(elem).getElementsBySelector('.product-afmeting').each(function(afmeting) {
        $afmeting = '<span>' + afmeting.innerHTML + '</span><br />';
    });
    ...
    if ($(elem).getElementsBySelector('.increment').length) {
        var increment = parseFloat($(elem).getElementsBySelector('.increment')[0].value);
        if (increment > 0) {
            jQuery('#fastorder-' + result_id).find('.fastorder-row-qty .qty').attr('data-increment', increment).val(increment);
        }
    }
    if ($(elem).getElementsBySelector('.minsale').length) {
        var minsale = parseFloat($(elem).getElementsBySelector('.minsale')[0].value);
        if (minsale > 0) {
            jQuery('#fastorder-' + result_id).find('.fastorder-row-qty .qty').val(minsale);
        }
    }
    if($tierprice && $tierprice != 'undefined'){
        $$('#fastorder-'+result_id+' .fastorder-row-qty .price_group').each(function(tierprice) {
            $(tierprice).update($tierprice);
        });
    }

    $$('#fastorder-'+result_id+' .fastorder-row-image').each(function(img, index) {
        if (!img) {
            img = index;
        }
        $(img).update($img);
    });
    $$('#fastorder-'+result_id+' .fastorder-row-name').each(function(name, index) {
        if (!name) {
            name = index;
        }
        $(name).update('<div><a class="animation" target="_blank" href="'+ $url +'">'+$name+'</a>'+'<br><span class="reference"></span>' + $afmeting + $price + '</div><div class="row-loader"></div>');
    });
    ...
    $$('#fastorder-'+result_id+' input.sku').each(function(input, index) {
        if (!input) {
            input = index;
        }
        $(input).value = sku;
    });
    $$('#fastorder-'+result_id+' input.typeid').each(function(input, index) {
        if (!input) {
            input = index;
        }
        $(input).value = $typeid;
    ...
    //animate image & name
    $$('#fastorder-'+result_id+' .fastorder-row-image div.animation').each(function(animation, index) {
        if (!animation) {
            animation = index;
        }
        new Effect.Morph(
            $(animation), 
            {
                ...
                afterFinish: 
                function() {
                    $$('#fastorder-'+result_id+' .fastorder-row-name div.animation').each(function(animation2, index2) {
                        if (!animation2) {
                            animation2 = index2;
                        }
                        new Effect.Morph(
...
jQuery(document).ready(function() {
    ...
    jQuery(document).on('change','#fastorder .fastorder-row-qty .qty',function(){
        ...
        var sku = jQuery(this).next().next().val();
        var typeid = jQuery(this).next().next().next().val();
