...
jQuery(document).ready(function() {
	jQuery(textOptions.resizable_text_class).fitText(5.2, { minFontSize: '10px', maxFontSize: '18px' });
});
...
jQuery(document).ready(function () {
	...
	jQuery(cms_banner_block).each(function(){
		var background_image = jQuery(this).find(' > img ');
		if(typeof background_image !=='undefined')
		{	
			var src = background_image.attr('src');
			jQuery(this).css("background", "transparent url('"+src+"') no-repeat center center");
...
$j(window).load(function() {   
	$j(".featured-brands-d3 ul").owlCarousel({
		items : 5,
		itemsCustom : false,
		itemsDesktop : [1199,4],
