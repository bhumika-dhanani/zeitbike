...
IWD.OPC = {
    ...
    initSaveOrder: function () {
        ...
        $j_opc(document).on('click touch', '.opc-btn-checkout', function () {
    ...
    initPayment: function () {
        ...
        $j_opc(document).on('click touch', '#co-payment-form input[type="radio"]', function (event) {
            IWD.OPC.removeNotAllowedPaymentMethods();
            IWD.OPC.validatePayment();
...
//                var requiredForShipperHQFields = form.find('[name="'+form_type+'[region_id]"]', '[name="'+form_type+':country_id]"]', '[name="'+form_type+':postcode]"]');
    ...
    updateShippingMethods: function() {
        $j_opc(document).on('click touch', '#update-shipping-methods', function(e){
...
IWD.OPC.Coupon = {
    init: function () {
        ...
        $j_opc(document).on('click touch', '.apply-coupon', function () {
            IWD.OPC.Coupon.applyCoupon(false);
        });


        $j_opc(document).on('click touch', '.remove-coupon', function () {
            IWD.OPC.Coupon.applyCoupon(true);
        });


        $j_opc(document).on('click touch', '.discount-block h3', function () {
...
IWD.OPC.Comment = {
    init: function () {
        ...
        $j_opc(document).on('click touch', '.comment-block h3', function () {
            if ($j_opc(this).hasClass('open-block')) {
                $j_opc(this).removeClass('open-block');
                $j_opc(this).next().addClass('hidden');
            } else {
...
IWD.OPC.SignatureAtCheckout = {
    init: function () {
        $j_opc(document).on('click touch', '.signature-block h3', function () {
            if ($j_opc(this).hasClass('open-block')) {
                $j_opc(this).removeClass('open-block');
                $j_opc(this).next().addClass('hidden');
            } else {
...
IWD.OPC.Agreement = {
    ...
    init: function () {
        ...
        $j_opc(document).on('click touch', '.view-agreement', function (e) {
            e.preventDefault();
        ...
        $j_opc(document).on('click touch', '#checkout-agreements input[name*="agreement"]', function () {
...
IWD.OPC.Login = {
    ...
    init: function () {
        ...
        $j_opc(document).on('click touch', '.md-modal .close', function (e) {
            e.preventDefault();
            $j_opc('.md-modal').removeClass('md-show');
        });

        $j_opc(document).on('click touch', '.md-overlay', function (e) {
            e.preventDefault();
            $j_opc('.md-modal').removeClass('md-show');
        });

        $j_opc(document).on('click touch', '.restore-account', function (e) {
            e.preventDefault();
        ...
        if ($j_opc('.opc-login-trigger').hasClass('is_persistent')) {
            $j_opc('.signin-modal').trigger('click touch');
        }
...
IWD.OPC.Decorator = {
    ...
    setActivePayment: function () {
        ...
        $j_opc(document).on('click touch', '#checkout-payment-method-load dt', function () {
    ...
    decorateCreditCardType: function (container) {
        ...
        $j_opc(".opc-cc-list li").on('click touch', function (e) {
...
$j_opc(document).ready(function () {
    ...
    if ($j('#is_subscribed').is(':checked'))
        $j('#is_subscribed').parent().prepend('<i class="fa fa-check-square"></i>');
    else
        $j('#is_subscribed').parent().prepend('<i class="fa fa-square-o"></i>');

    if ($j('.form-list .create_account').is(':checked'))
        $j('.form-list .create_account').parent().prepend('<i class="fa fa-check-square"></i>');
    else
        $j('.form-list .create_account').parent().prepend('<i class="fa fa-square-o"></i>');

    if ($j('.form-list .use_for_shipping_yes').is(':checked'))
        $j('.form-list .use_for_shipping_yes').parent().prepend('<i class="fa fa-check-square"></i>');
    else
        $j('.form-list .use_for_shipping_yes').parent().prepend('<i class="fa fa-square-o"></i>');


    $j('.opc-newsletter .control .checkbox').on('click touch',function(){
    ...
    $j('.form-list .create_account').on('click touch',function(){
        if ($j(this).is(':checked')) {
            $j(this).parent().find('i').attr('class', '');
            $j(this).parent().find('i').addClass('fa fa-check-square');
        }
    ...
    $j('.form-list .use_for_shipping_yes').on('click touch',function(){
        if ($j(this).is(':checked')) {
            $j(this).parent().find('i').attr('class', '');
            $j(this).parent().find('i').addClass('fa fa-check-square');
        }
