...
IWD.Newspopup = {
	...
	popupInit: function(){
        ...
        $q(IWD.Newspopup.popupSelector).find('button.close').click(function(){
            $q(IWD.Newspopup.popupSelector).hide();
            IWD.Newspopup.hideLoaderBg();
		});
	...
	$q(document).ready(function(){
		...
		//alert(window.height);
		//$q('#popup-newssubscribe .news_popup').css({'max-height':$q(window).height()*0.8});
		////$q("popup-newssubscribe .news_popup").perfectScrollbar();
		//$q( window ).resize(function() {
		//	$q('#popup-newssubscribe .news_popup').css({'max-height':$q(window).height()*0.8});
		//	//$q('popup-newssubscribe .news_popup').perfectScrollbar('update');
		//});
