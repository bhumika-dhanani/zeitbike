...
class Twitter
{
    ...
    public function reconfigure($config = array())
    {
        ...
        $this->config = array_merge(array(
            ...
            'consumer_key' => Mage::getStoreConfig(IWD_Signin_Helper_Data::XML_PATH_TWITTER_KEY),
            'consumer_secret' => Mage::getStoreConfig(IWD_Signin_Helper_Data::XML_PATH_TWITTER_SECRET),
            'token' => '',
            'secret' => '',
...
, $config);
     ...
     *
     * @param string $length
     *            how many characters the nonce should be before MD5 hashing. default 12
     * @param string $include_time
     *            whether to include time at the beginning of the nonce. default true
     ...
     * we chose a space here as twitter whitespace trims from
     ...
     * @param array $params
     *            the request parameters as an array of key=value pairs. Default empty array
     * @param string $useauth
     *            whether to use authentication when making the request. Default true
     * @param string $multipart
     *            whether this request contains multipart data. Default false
     * @param array $headers
     *            any custom headers to send with the request. Default empty array
    ...
    /**
     * Signs the request and adds the OAuth signature.
     * This runs all the request
     ...
     *            whether to include the user credentials when making the request.
     * @return void
     */
    ...
    /**
     * Make a long poll HTTP request using this library.
     * This method is
     ...
     *            the callback function to stream the buffer to.
     * @return void
     */
     ...
     * @param string $mode
     *            the transformation mode. either encode or decode
    ...
    private function curlHeader($ch, $header)
    {
        ...
        if (! isset($this->response['headers'][$key])) {
            ...
            if (! is_array($this->response['headers'][$key])) {
                $this->response['headers'][$key] = array(
