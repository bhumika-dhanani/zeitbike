	...
	{	
		...
		if($status!="0"){ //login name invalid
			...
			if($version!==0)
			{
				...
				{
					$responseArray['StatusMessage'] = $message ." However, your store version is " . $version ." which hasn't been fully tested with Webgility. If you'd still like to continue, click OK to continue or contact Webgility to confirm compatibility.";
				}else{
					$responseArray['StatusMessage'] = $message;
				}	
			}else{
				$responseArray['StatusMessage'] = $message." However, Webgility is unable to detect your store version. If you'd still like to continue, click OK to continue or contact Webgility to confirm compatibility.";
			}
		}
...
function saveShipment($username,$password,$xmlShipmentItem,$storeid=1)
{
		...
		if($status!="0"){ //login name invalid
			...
			foreach($orders_array_w as $orders_el){	
				...
				->load($orders_el['entity_id']);
					...
					if($emailAlert=='Y') {
						$_SERVER['SCRIPT_FILENAME'] = str_replace("webgility-magento","index",$_SERVER['SCRIPT_FILENAME']);
						$_SERVER['REQUEST_URI'] = str_replace("webgility-magento","index",$_SERVER['REQUEST_URI']);
						$_SERVER['PHP_SELF'] = str_replace("webgility-magento","index",$_SERVER['PHP_SELF']);
						$_SERVER['SCRIPT_NAME'] = str_replace("webgility-magento","index",$_SERVER['SCRIPT_NAME']);
	...
	function getStores($username,$password,$devicetokenid)
	{
		...
		$ecc_base_path = Mage::getBaseDir('base').'/webgility';
