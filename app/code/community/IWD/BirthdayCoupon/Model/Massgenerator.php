 ...
 */
class IWD_BirthdayCoupon_Model_Massgenerator extends Mage_Core_Model_Abstract
    implements Mage_SalesRule_Model_Coupon_CodegeneratorInterface
{
    ...
    public function generateCode()
    {
        ...
        if (!$format) {
            $format = IWD_BirthdayCoupon_Helper_Coupons::COUPON_FORMAT_ALPHANUMERIC;
        }
        ...
        $charset = Mage::helper('iwd_coupon/coupons')->getCharset($format);
    ...
    public function getDelimiter()
    {
        if ($this->getData('delimiter')) {
            ...
            return Mage::helper('iwd_coupon/coupons')->getCodeSeparator();
        }
    }
    ...
    public function generatePool()
    {
        ...
        $chars = count(Mage::helper('iwd_coupon/coupons')->getCharset($this->getFormat()));
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            do {
                if ($attempt >= $maxAttempts) {
                    Mage::throwException(Mage::helper('iwd_coupon')->__('Unable to create requested Coupon Qty. Please check settings and try again.'));
                }
