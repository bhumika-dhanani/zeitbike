<?php
class IWD_AutoRelatedProducts_Block_Adminhtml_Blocks_Edit_Tab_Featured extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('conditions_fieldset', array(
            'legend' =>$helper->__('Set the conditions for the choice of featured products'))
        ...
        $fieldset_grid = $form->addFieldset('related_grid_fieldset', array(
            'legend' => $helper->__('Or select featured products manually'),
            'class'=>'iwd_fieldset_grid'
        ));

        $featured_products_grid = $this->getLayout()->createBlock('iwd_autorelatedproducts/adminhtml_blocks_edit_grid_featured', 'iwd_autorelatedproducts.featured.grid')->toHtml();
        $serialize_block = $this->getLayout()->createBlock('adminhtml/widget_grid_serializer');
        $serialize_block->initSerializerBlock('iwd_autorelatedproducts.featured.grid', 'getSelectedProducts', 'featured_products', 'selected_products');
        $featured_products_grid .= $serialize_block->toHtml();

        $fieldset_grid->addField('related_grid', 'note', array(
            'name' => 'related_grid',
            'text' => $featured_products_grid,
        ));
