<?php
class IWD_Signin_Helper_Paypal extends Mage_Customer_Helper_Data {
	
	const RETURN_URL = 'signin/openidconnect/return';
	
	const XML_PATH_ENABLED = 'signin/paypallogin/status';
	
	const XML_PATH_SANDBOX_MODE = 'customer/startup/paypalauth_sandboxenabled';
	
	const XML_PATH_CLIENT_ID = 'signin/paypallogin/clientid';
	
	const XML_PATH_CLIENT_SECRET = 'signin/paypallogin/secret';
	...
	public function __construct() {
		...
		$this->_sandbox = Mage::getStoreConfig ( 'signin/paypallogin/sandbox' );
	...
	public function linkAccounts($_profile, $customer) {
		...
		$paypalCustomer = Mage::getModel ( 'signin/paypal_customer' )->load ( $customerId, 'customer_id' );
	...
	public function getPopUpLoginUrl() {
		return $this->_getUrl ( 'signin/openidconnect/login', array (
				'_secure' => true 
		) );
	}
	...
	public function getAskLinkPostActionUrl() {
		...
		return $this->_getUrl ( 'signin/openidconnect/askLinkPost', $params );
	}
