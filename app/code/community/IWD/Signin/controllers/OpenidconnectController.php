<?php
class IWD_Signin_OpenidconnectController extends Mage_Core_Controller_Front_Action {
	...
	private  function _getHelper() {
		return Mage::helper ( 'signin/paypal' );
	}
	...
	public function returnAction() {
		...
		try{
			...
			if (isset($_params['code'])){
				...
				if (isset ( $_params ['error_uri'] )) {
					...
					//$this->_getSession ()->addError ( $this->__("Login to PayPal got cancelled or failed! ") ); 
		...
		$paypalCustomerData = Mage::getModel ( 'signin/paypal_customer' )->getPaypalCustomerDataByField ( 'payer_id', $profile->getUserId() );
		...
		if (! $paypalCustomerData) {
			...
			if ($this->_getSession ()->isLoggedIn ()) {
				...
				$url = Mage::getUrl ( 'signin/openidconnect/askLink' );
			...
			$paypalCustomer = Mage::getModel ( 'signin/paypal_customer' );
				...
				try {
					...
					switch ($e->getCode ()) {
						...
						case Mage_Customer_Model_Customer::EXCEPTION_INVALID_EMAIL_OR_PASSWORD :
							...
							$redirectUrl = 'login/openidconnect/askLink';
							break;
						default :
							$message = $e->getMessage ();
					}
	...
	public function unlinkAction() {
		if ($this->_getSession ()->getCustomer ()->getId ()) {
			$paypalCustomer = Mage::getModel ( 'signin/paypal_customer' )->unlinkAccount ();
	...
	public function askLinkPostAction() {
		...
		$askLinkUrl = 'signin/openidconnect/askLink';
		...
		if (array_key_exists ( 'send', $params )) {
			...
			$isPaypalCustomerExists = Mage::getModel ( 'signin/paypal_customer' )->isPaypalCustomerExists ( 'customer_id', $customerId );
			...
			if ($url == 'signin/openidconnect/askLink') {
				$this->_redirect ( $url );
				return;
			}
