<?php
class IWD_FeaturedProducts_Block_Adminhtml_List_Grid extends Mage_Adminhtml_Block_Widget_Grid {
	
	public function __construct() {
		parent::__construct ();		
		$this->setId ( 'iwd_featured_products' );
	...
	protected function _prepareCollection() {
		...
		$collection = Mage::getModel ( 'catalog/product' )->getCollection ()->addAttributeToSelect ( 'name' )->addAttributeToSelect ( 'sku' )->addAttributeToSelect ( 'iwd_featured_product' )->addAttributeToSelect ( 'type_id' )->addAttributeToFilter ( 'visibility', array ('nin' => array (1, 3 ) ) );
	...
	protected function _prepareColumns() {
		$this->addColumn ( 'featured', array ('header_css_class' => 'a-center', 'type' => 'checkbox', 'name' => 'featured', 'values' => $this->_getSelectedProducts (), 'align' => 'center', 'index' => 'entity_id' ) );
		...
		$this->addColumn ( 'sku', array ('header' => Mage::helper ( 'catalog' )->__ ( 'SKU' ), 'width' => '140', 'index' => 'sku' ) );		
	...
	protected function _getSelectedProducts($json = false) {
		...
		$_product = Mage::getModel ( 'catalog/product' )->getCollection ()->joinAttribute ( 'iwd_featured_product', 'catalog_product/zest_featured_product', 'entity_id', null, 'left', $store->getId () )->addAttributeToFilter ( 'zest_featured_product', '1' );
		
		$products = $_product->getColumnValues ( 'entity_id' );
...
EndHTML;
	...
	private function _appendHtml() {
		...
		var checkbox_all = $$("#iwd_featured_products_table thead input.checkbox").first();
		var everycheckbox = $$("#iwd_featured_products_table tbody input.checkbox");
