	...
	public function automaticallyInvoiceShipCompleteOrder($observer) {
	   try {
		  ...
		  //Mage::log('automatically send invoice'.$this->_invoice, null, 'bsslog1.log');
           ...
           $invoiceData = Mage::getModel("sales/order_invoice")->load($this->_invoice->getId());
           $orderData = Mage::getModel('sales/order')->load($invoiceData->getOrderId());

           $statusArray = array("complete","closed");
           if(!in_array($orderData->getStatus(),$statusArray)) {
               if ($invoiceData->getEmailSent() == '' || is_null($invoiceData->getEmailSent())) {
