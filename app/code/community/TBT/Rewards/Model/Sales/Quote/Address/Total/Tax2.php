...
class TBT_Rewards_Model_Sales_Quote_Address_Total_Tax2 extends Mage_Tax_Model_Sales_Total_Quote_Tax
{
    ...
    protected function _getRewardsTaxDiscount($address)
    {
        $items         = $this->_getAddressItems($address);
        $store         = $address->getQuote()->getStore();
        $pricesInclTax =  Mage::helper('tax')->priceIncludesTax($store);

        $total_discount_tax_amount = 0;

        foreach ($items as $item) {
            if ($pricesInclTax) {
                $catalog_discount_w_tax = $this->_getRedeemer()->getTotalCatalogDiscount($item);
                $catalog_discount       = $catalog_discount_w_tax / (1 + $item->getTaxPercent() / 100);
            } else {
        ...
        return $this->_round($address, $total_discount_tax_amount, $pricesInclTax);
    }
    ...
    protected function _addRoundingDelta($address, $raw_amount, $rounded_amount)
    {
        ...
        $rewardsRoundingDelta = $address->getRewardsRoundingDelta() == null ? 0 : $address->getRewardsRoundingDelta();
        $address->setRewardsRoundingDelta($delta + $rewardsRoundingDelta);

        return $this;
    }

    /**
     * This is used to make sure that STR catalog discounts don't introduce some 0.01 tax total issues.
     * For this we take into account Magento's rounding deltas.
     *
     * @param  Mage_Sales_Model_Quote_Address $address [description]
     * @param  float $amount        Amount to round
     * @param  bool $direction      Prices incl tax or not
     * @return float
     */
    protected function _round($address, $amount, $direction)
    {
        if (!$amount) {
            return $this->_calculator->round($amount);
        }

        $calculator         = $this->_calculator;
        $rate               = (string)$calculator->getRateValue();
        $calculationProcess = $this->_calculator->getCalculationProcess();
        $rateKey    = isset($calculationProcess[0])
            ? (string)$calculationProcess[0]['id']
            : $rate;
        $deltas     = $address->getRoundingDeltas();
        $key        = 'regular' . $direction;
        $delta = null;
        if (array_key_exists($key, $deltas) ) {
            if (array_key_exists($rate, $deltas[$key]) ) {
                $delta = isset($deltas[$key][$rate]) ? $deltas[$key][$rate] : $deltas[$key][$rateKey];
            }
        }
        $delta = !is_null($delta) ? $delta : 0.000001;
        $amount     = $amount + $delta;
