    ...
    private function _loadPointsCollections()
    {
        ...
        try {
            ...
            if (Mage::helper('rewards/customer_points_index')->useIndex()) {
                ...
                if(!isset($balanceData['customer_points_usable'])) $balanceData['customer_points_usable'] = 0;
                $this->usable_points = array(1 => (int)$balanceData['customer_points_usable']);

                if(!isset($balanceData['customer_points_active'])) $balanceData['customer_points_active'] = 0;
                $this->points = array(1 => (int)$balanceData['customer_points_active']);

                if(!isset($balanceData['customer_points_pending_approval'])) $balanceData['customer_points_pending_approval'] = 0;
                $this->on_hold_points = array(1 => (int)$balanceData['customer_points_pending_approval']);

                if(!isset($balanceData['customer_points_pending_event'])) $balanceData['customer_points_pending_event'] = 0;
                $this->pending_points = array(1 => (int)$balanceData['customer_points_pending_event']);

                if(!isset($balanceData['customer_points_pending_time'])) $balanceData['customer_points_pending_time'] = 0;
