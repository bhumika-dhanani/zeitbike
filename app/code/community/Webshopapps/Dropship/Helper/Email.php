...
class Webshopapps_Dropship_Helper_Email extends Mage_Core_Helper_Abstract
{
	...
	public function salesOrderSaveAfter($observer){
		...
		if ($eventName == 'sales_order_place_after' || $eventName == 'rumbleship_payment_processing') {
        ...
        $excludedWarehouse = Mage::getStoreConfig('sales_email/dropship_order/exclude_warehouse');
        ...
        if (!empty($encDetails) && Mage::helper('dropcommon')->calculateDropshipRates()) {//DROP-98
            ...
            $sendEmailToExcludedWarehouse = false;
	         ...
	         foreach ($shipDetails as $shipItem) {
				...
				if(!$emailDetails['manualship']){
                    //Don't Create Shipment automatically for Zeitbike (CA) Warehouse
                    if($warehouse != $excludedWarehouse) {
						$shipment = $this->createShipment($order,$warehouse,$order->getShippingDescription());
                        foreach ($shipment->getAllItems() as $_item) {
                            $shippedOrderItemIds[] = $_item->getData('order_item_id');
                        }
                    } else {
                        $sendEmailToExcludedWarehouse = true;
                    }
...
//        			$this->sendFullShipmentEmail($warehouse,$shipment,$order, false,'', $withoutShipment);
                 }
	        }

            foreach ($order->getAllItems() as $_item) {
                if (!in_array($_item->getId(), $shippedOrderItemIds)) {
                    $_item->setData('_flag_not_shipped', true);
                }
            }

            if($sendEmailToExcludedWarehouse) {
                $emailQueue[] = array(
            ...
            foreach ($emailQueue as $email) {
                $this->sendFullShipmentEmail($email['warehouse'],$email['shipment'],$email['order'],$email['override_manual'],$email['comment']);
            }
...
//        			$this->sendFullShipmentEmail($warehouse,$shipment,$order, false,'', $withoutShipment);
    ...
    private function prepareShipment($warehouse, $qtys = array())
    {
        ...
        foreach ($this->_order->getAllItems() as $orderItem) {
            ...
            if ($orderItem->isDummy()) {
                $qty = $orderItem->getQtyOrdered() ? $orderItem->getQtyOrdered() : 0;
            } else {
 ...
 //   public function sendNewShipmentEmail(Mage_Sales_Model_Order_Shipment $shipment) {
    ...
    private function sendFullShipmentEmail($warehouse,$shipment,$order,  $overrideManual = false, $comment='')
    {
        ...
        if($attachPdf && $warehouse != $excludedWarehouse)
        {
        ...
        foreach ($sendTo as $recipient) {
            ...
            $mailTemplate->setDesignConfig(array('area'=>'frontend', 'store'=>$order->getStoreId()))
                ->sendTransactional(
                    ...
                    $recipient['name'],
                    array(
                        'template'    => !$shipment ? 'webshopapps/dropship/email/order/shipment/order_items.phtml' : 'webshopapps/dropship/email/order/shipment/shipment_items.phtml',
                        'order'       => $order,
                        'shipment'    => $shipment,
