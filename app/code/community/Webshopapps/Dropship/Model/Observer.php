...
class Webshopapps_Dropship_Model_Observer extends Mage_Core_Model_Abstract
{
    ...
    public function saveOrderAfter($observer)
    {
        try {
			...
			// Not place dropshipment after order place 
			if($eventName == 'sales_order_place_after' ){
				$order = $observer->getEvent()->getOrder();
				$payment_method_code = $order->getPayment()->getMethodInstance()->getCode(); 
				if($payment_method_code == 'rumbleship_payments'){
					return;
				}
			}
	...
	public function orderProcessAfter($observer)
	{
		$order = $observer->getEvent()->getOrder();
		$payment_method_code = $order->getPayment()->getMethodInstance()->getCode(); 
		...
		//	Mage::log("ORG:-".$order->getOrigData('status')." NEW:-".$order->getStatus());
		
			if(
				$payment_method_code == 'rumbleship_payments' && 
				$order->getStatus() == Mage_Sales_Model_Order::STATE_PROCESSING && $createEmail == 'order' && 
				$order->getOrigData('status') == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT
			){
    ...
    public function preDispatchShippingMethodSave($observer)
    {
        ...
        if ($controller->getRequest()->isPost()) {
            ...
            foreach (array('dropship_date_input','dropship_extrainfo_input') as $dropshipExclId) {
                if (array_key_exists($dropshipExclId,$data)) {
                    unset($data[$dropshipExclId]);  // remove additional parameters
                }
            }
            ...
            */
            ...
            $controller->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
            if (! Mage::getStoreConfig('opc/global/status')) {
                $controller->setFlag('', Mage_Checkout_OnepageController::FLAG_NO_DISPATCH, true);
            }else{
                $controller->setFlag('', IWD_Opc_JsonController::FLAG_NO_DISPATCH, true);
