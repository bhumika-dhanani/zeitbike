    ...
    public function _addAssociatedProductFilters()
    {
        ...
        if(method_exists($product,'getTypeInstance')) {
            ...
            if(method_exists($typeInstance,'getUsedProducts')) {
                $typeInstance->getUsedProducts($this->getColumnValues('attribute_id'), $this->getProduct()); //Filter associated products
            }
        }
        return $this;
    }
        ...
        if ($this->count()) {
            ...
            foreach ($this->_items as $item) {
                ...
                if(method_exists($product,'getTypeInstance')) {
                    $typeInstance = $product->getTypeInstance(true);
                    if(method_exists($typeInstance,'getUsedProducts')) {
                        $typeInstance->getUsedProducts(array($productAttribute->getAttributeCode()), $this->getProduct()); //Filter associated products
