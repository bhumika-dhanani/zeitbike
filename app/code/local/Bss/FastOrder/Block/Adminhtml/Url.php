 ...
 */
class Bss_FastOrder_Block_Adminhtml_Url extends Mage_Adminhtml_Block_System_Config_Form_Field {
    ...
    protected function _getHeaderHtml() {
        ...
        $output .= Mage::helper('fastorder')->__('ID/Class selector of block to be updated');
        $output .= '</th>';
        $output .= '<th style="padding: 2px; text-align: center;">';
        $output .= Mage::helper('fastorder')->__('Layout Update Block name(should be same as in XML)');
        $output .= '</th>';
        $output .= '<th>&nbsp;</th>';
        $output .= '</tr>';
        return $output;
