...
class Amasty_Stockstatus_Model_Observer
{
    ...
    public function onListBlockHtmlBefore($observer)//core_block_abstract_to_html_after    
    {
        $html = $observer->getTransport()->getHtml();
        if ($html
            && $observer->getBlock() instanceof Mage_Catalog_Block_Product_List
            && Mage::getStoreConfig('amstockstatus/general/display_at_categoty')
        ) {
            preg_match_all('/price[a-z\-]*?([0-9]*?)"/', $html, $productsId);

            if ($productsId && isset($productsId[1])) {
                $ids = array_unique($productsId[1]);
                $collection = $this->getCollectionByIds($ids);
                $results = $this->getPricesHtml($html, implode('|', $collection->getAllIds()));

                if ($results && isset($results[0])) {
                    foreach ($results[0] as $key => $result) {
                        if (isset($results[2][$key])) {
                            $stockStatus = Mage::helper('amstockstatus')->showStockStatus(
                                $collection->getItemById($results[2][$key]),
                                false,
                                true
                            );
                            if ($stockStatus) {
                                $replace = $result . $stockStatus;
                                $html = str_replace($result, $replace, $html);
                            }
                        }
                    }
     ...
     */
    protected function getCollectionByIds($ids)
    {
    ...
    /**
     * @param string $html
     * @param string $idsCondition
     * @return array|null
     */
    protected function getPricesHtml($html, $idsCondition)
    {
        $results = null;
        $templates = array(
            '@(product-price-(' . $idsCondition . ')">(.*?)div>)@s',
            '@(price-including-tax-(' . $idsCondition . ')">(.*?)div>)@s',
            '@(price-excluding-tax-(' . $idsCondition . ')">(.*?)div>)@s'
        );

        foreach ($templates as $template) {
            preg_match_all($template, $html, $results);
            if ($results) {
                break;
            }
        }
