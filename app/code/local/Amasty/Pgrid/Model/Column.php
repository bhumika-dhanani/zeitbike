...
class Amasty_Pgrid_Model_Column extends Mage_Core_Model_Abstract
{
    ...
    {
        return $this->getCustomTitle() ? $this->getCustomTitle() : $this->getData('title');
    }
    ...
    public function isDisplayTotals()
    {
        $isDisplayTotals = $this->getIsDisplayTotals();
        if (!$this->getDisplayTotals()) {
            $isDisplayTotals = false;
        } elseif (!$this->getColumnId() && $this->getColumnType() != 'standard') {
            $isDisplayTotals = false;
        }
        return $isDisplayTotals;
    }
}
